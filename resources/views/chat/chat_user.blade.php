        ...
        <div
            ...
            <div class="flex sm:items-center justify-between py-3 border-b-2 border-gray-200">
                <div class="relative flex items-center space-x-4">
                    <div class="relative outline-dotted outline-2 outline-offset-2 outline-emerald-300  rounded-full">
                        ...
                        <img src="{{ $user->profile_photo_path ?? asset('images/pngwing.png') }}" alt="lo go "
                            class="w-10 sm:w-16 h-10 sm:h-16 rounded-full">
                    </div>
                    <div class="flex flex-col leading-tight">
                        <div class="text-2xl mt-1 flex items-center">
                            <span class="text-gray-700 mr-3">Room chat User</span>
                        </div>
            ...
            <div class="border-t-2 border-gray-200 px-4 pt-4 mb-2 sm:mb-0">
                <div class="relative flex">
                    ...
                    <input type="text" placeholder="Write your message!" id="input-message"
                        class="w-full focus:outline-none focus:placeholder-gray-400 text-gray-600 placeholder-gray-600 pl-12 bg-gray-200 rounded-md py-3 border-none">
                    <div class="absolute right-0 items-center inset-y-0 hidden sm:flex">
                        ...
                        <button type="button" data-modal-target="authentication-modal"
                            class="modal uploadImage inline-flex items-center justify-center rounded-full h-10 w-10 transition duration-500 ease-in-out text-gray-500 hover:bg-gray-300 focus:outline-none">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor" class="h-6 w-6 text-gray-600">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <button type="button" id="btn-chat"
...
{{-- <script>
    ...
    $(function() {
        let messages = {};
        let ip_address = "{{ env('DB_HOST') }}";
        let socket_port = "{{ env('PORT_SERVE_NODE') }}";
        let socket = io(ip_address + ':' + socket_port);
        const userInRoom = [{{ auth()->user()->id }}, {{ $user->id }}];
        userInRoom.sort();
        const chatRoom = userInRoom[0] + "_room_" + userInRoom[1];
        const user = {
            id: {{ auth()->user()->id }},
            name: "{{ auth()->user()->name }}",
        ...
        // Join room 
        socket.emit('joinRoom', chatRoom, user.name);
        ...
        function listMessage(message) {
            ...
            if (message.leave) {
                ...
                data = `<div class="chat-message">
                            <div class="flex items-end ${message.from === user.id ? 'justify-end' : ''}">
                                <div class="flex flex-col space-y-2 text-xs max-w-xs mx-2 order-2 items-start">
                                <div>
                                    <span class="px-4 py-2 rounded-lg inline-block rounded-bl-none ${message.from === user.id ? 'bg-blue-600 text-white' : 'bg-gray-300 text-gray-600'}">${message.content}</span></div>
                                </div>
...
<script>
    ...
    const urlGetMessage = "{{ route('chat_user', $user->id) }}";
    const urlSend = "{{ route('chat_message_create') }}"
</script>
